...
| 4 | PlatformLead Triage Tools | In progress | — | `bcf22be`, `e10b8bc`, `55243f3`, `3b49965`; current local slice pending deploy/QA | Status and notes slices shipped: admin can open a stable PlatformLead detail route, update status, add internal notes, see notes in the lead timeline, and audit both mutations. Added assignment-to-current-admin control alongside follow-up/source ops fields. Current local slice adds list-level owner-ops/artifact visibility and a richer reverse-chronological timeline across intake jobs, AI artifacts, approvals, notes, preview builds, follow-ups, spam review, and contract milestones. Local lint/build passed. Remaining: production deploy/QA for latest triage polish. |
