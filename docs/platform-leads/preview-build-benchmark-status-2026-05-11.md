...
The next DirectStay Preview Build refinement heartbeat can render a constrained **internal-only Sarasota benchmark** from `sarasota-river-retreat-render-fixture-packet-2026-05-14.md` or the no-write JSON export at `node directstay/scripts/create-sarasota-river-retreat-preview-fixture.mjs --json` using a local fixture/test-seed path. Do not create production database rows, share an owner-facing Sarasota preview, or use exterior/amenity imagery until an owner/gallery photo set supports safe hero and early-section art.
