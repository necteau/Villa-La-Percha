# Villa La Percha Guest Rental Agreement — Owner Review Draft

> Draft only for Jaimal/owner/counsel review. Not legal advice. Do not send as a final or enforceable agreement, present to guests, or activate in a booking/signature workflow until approved by Jaimal and, ideally, qualified counsel.

## Reservation Information Summary

The final guest-facing version should generate these fields from the booking record:
...
The Property is expected to include a reasonable supply of linens, beach towels, kitchenware, televisions, and washer/dryer access, together with amenities listed in the booking confirmation.
...
The prior source contract capped Owner/Agent liability at the total rental amount paid by Guest and required claims/proceedings to begin within six months after the rental start date. Counsel should confirm whether to preserve, modify, or remove those provisions in the final approved agreement.
...
Recommended owner-review placeholder:
...
The acceptance record should include, where available:

- agreement version/date/hash;
- primary guest name and contact information;
- acceptance/signature timestamp;
- accepted reservation dates and payment terms;
...
## Owner/Counsel Review Checklist
...
11. Governing law, venue, attorneys’ fees, arbitration/mediation if desired.
12. Any local short-term rental, consumer, occupancy, insurance, or safety disclosures.
13. Plain-language guest summary for booking checkout before acceptance.

## DirectStay Integration Notes

These notes are not guest-facing contract language.

Recommended product states:

- `contract_not_required` — reservation does not require a contract.
- `contract_pending` — contract required but not yet sent/accepted.
- `contract_sent` — guest has received/review link was generated.
- `contract_viewed` — optional event if tracked.
- `contract_accepted` — accepted/signed with immutable version record.
- `contract_voided` — superseded by new version or reservation change.
- `contract_declined` — guest declined or booking cancelled before acceptance.

Reservation timeline events should record:

- agreement version created;
- agreement sent to guest;
- guest viewed/opened link if available;
- guest accepted/signed;
- contract superseded after reservation changes;
- owner/admin manually marked reviewed, if needed.

Guest-facing checkout should highlight, before acceptance:

- nonrefundable deposit/down payment;
- cancellation deadline and fee;
- final payment due date;
- $1,500 security deposit/card hold if retained;
- max occupancy of 8 people present at any time;
- no parties/events/weddings/reunions above occupancy;
