...
export async function getPreviewBuildGateReport(previewBuildId: string) {
  ...
  const imageInventoryTerms = ["Page-order image inventory", "Hero candidate", "First two section-image candidates", "Rejected assets", "Public listing/OTA galleries checked"];
  ...
  const readyBlockers = [
    ...
    ...(!approvedPhotoGeoAuditHasImageInventory ? ["Approved photo/geography audit must replace starter TODOs with a page-order image inventory, hero candidate, first two section-image candidates, rejected assets, and public listing/OTA gallery check before READY FOR REVIEW. If credible photos cannot be found, mark image-blocked instead of building a real Preview Build."] : []),
