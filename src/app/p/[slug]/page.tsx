  ...
  return (
    <main style={{ fontFamily: "Inter, ui-sans-serif, system-ui", color: "#17211a", background: "#f7f3eb", minHeight: "100vh" }}>
      <section style={{ padding: "72px 24px", maxWidth: 1120, margin: "0 auto" }}>
        ...
        {showOwnerNotes && callouts[0] ? <aside style={{ marginTop: 32, padding: 18, border: "1px solid #d8c7a3", borderRadius: 18, background: "#fffaf0" }}><strong>Owner note: {callouts[0].label}</strong><p>{callouts[0].body}</p></aside> : null}
      </section>
      ...
      <section style={{ padding: "56px 24px", maxWidth: 960, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gap: 12, opacity: .82 }}>
        ...
        {showOwnerNotes && callouts.slice(1).map((callout, index) => <aside key={index} style={{ marginTop: 18, padding: 16, border: "1px solid #d8c7a3", borderRadius: 18, background: "#fffaf0" }}><strong>Owner note: {callout.label}</strong><p>{callout.body}</p></aside>)}
