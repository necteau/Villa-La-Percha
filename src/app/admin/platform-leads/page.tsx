  ...
  return (
    <div>
      ...
      <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Lead</th><th>Status</th><th>Property</th><th>Location</th><th>Created</th></tr></thead><tbody>
        {leads.map((lead) => <tr key={lead.id}><td><Link href={`/admin/platform-leads/${lead.id}`}>{lead.fullName}</Link><br/><span className="admin-muted">{lead.email}</span></td><td><span className="admin-chip">{lead.status}</span></td><td>{lead.propertyName || lead.company || "—"}</td><td>{lead.propertyLocation || "—"}</td><td>{lead.createdAt.toISOString().slice(0,10)}</td></tr>)}
      </tbody></table></div>
    </div>
  );
}
