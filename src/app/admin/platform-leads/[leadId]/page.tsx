  ...
  return (
    <div>
      ...
      <section className="admin-detail-grid admin-section">
        <article className="admin-card"><h3>Triage</h3><form action={`/admin/platform-leads/${lead.id}/status`} method="post" className="admin-form-stack"><label className="admin-muted" htmlFor="status">Lead status</label><select id="status" name="status" defaultValue={lead.status}>{PLATFORM_LEAD_STATUSES.map((status) => <option key={status} value={status}>{status.replaceAll("_", " ")}</option>)}</select><button type="submit">Update status</button></form></article>
