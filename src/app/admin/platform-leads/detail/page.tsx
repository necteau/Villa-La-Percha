...
const ARTIFACT_TYPES: PlatformLeadArtifactType[] = ["LEAD_BRIEF", "FIRST_RESPONSE_DRAFT", "PROPOSAL_RATIONALE", "PROPOSAL_DRAFT", "ONBOARDING_BRIEF", "ONBOARDING_EMAIL_DRAFT", "OWNER_PLATFORM_AGREEMENT"];
...
function reviewGuidance(status: PlatformLeadArtifactStatus) {
  if (status === "NEEDS_APPROVAL") return "Awaiting Jaimal review before any owner-facing action.";
  if (status === "APPROVED") return "Approved for use; still send manually outside DirectStay when ready.";
  if (status === "SENT") return "Marked sent after manual outreach; DirectStay did not send this automatically.";
  if (status === "REJECTED") return "Rejected; keep for audit trail and generate or write a replacement if needed.";
