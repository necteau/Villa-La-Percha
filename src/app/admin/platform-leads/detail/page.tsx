...
function previewSections(value: unknown): Array<{ kind?: string; eyebrow?: string; title?: string; heading?: string; body?: string; imageUrl?: string; imageAlt?: string }> {
  return Array.isArray(value) ? value as Array<{ kind?: string; eyebrow?: string; title?: string; heading?: string; body?: string; imageUrl?: string; imageAlt?: string }> : [];
}
function previewArtifactsByType(lead: NonNullable<Awaited<ReturnType<typeof getAdminPlatformLead>>>) {
  const latest = new Map<PlatformLeadArtifactType, NonNullable<Awaited<ReturnType<typeof getAdminPlatformLead>>>["artifacts"][number]>();
  for (const artifact of lead.artifacts) if (String(artifact.type).startsWith("PREVIEW_") && activePreviewArtifact(artifact) && !latest.has(artifact.type)) latest.set(artifact.type, artifact);
  return latest;
}

function previewJson(value: unknown, fallback: string) {
...
const DEFAULT_PREVIEW_SECTIONS = `[
  {
    ...
    "title": "What makes this stay feel specific",
    "body": "Replace this with the evidence-based property story from the photo/geography audit.",
    "items": [
      { "label": "Source-backed hook", "body": "Tie this detail to a visible photo, owner note, or exact location fact." }
    ]
  },
  {
    "kind": "signatureMoments",
    "eyebrow": "Signature moments",
    "title": "The moments guests should imagine first",
    "items": [
      { "label": "Arrival", "body": "Draft the arrival emotion from property evidence." },
      { "label": "Best hour", "body": "Name the time/place/light that the property owns." }
    ]
  },
  {
    "kind": "locationGuide",
    "eyebrow": "Micro-location",
    "title": "Why this exact location matters",
    "body": "Use micro-geography, not a generic region label."
  }
]`;

const DEFAULT_OWNER_CALLOUTS = `[
  {
    "label": "Property-specific strategy",
    "body": "Explain one design/content decision that only this property and location justify. This must be longer than generic onboarding copy."
  },
  {
    "label": "Assumptions to correct",
    "body": "List the assumptions the owner should confirm, replace, or remove before this can become launch content."
  }
]`;
...
export default async function AdminPlatformLeadDetailPage({ searchParams }: { searchParams: Promise<{ leadId?: string; previewGate?: string }> }) {
  ...
  const previewArtifactMap = previewArtifactsByType(lead);
  ...
  return <div>
    ...
    <section className="admin-section admin-card"><h3>Preview Build packet review</h3><p className="admin-muted">Guided review surface for the required packet. Draft artifacts are useful for generation; approved artifacts are required for owner sharing/promotion gates.</p><div className="admin-detail-grid">{(["PREVIEW_PHOTO_GEO_AUDIT", "PREVIEW_DESIGN_BRIEF", "PREVIEW_FACT_REGISTER", "PREVIEW_ASSUMPTION_REGISTER", "PREVIEW_RUBRIC_REVIEW", "PREVIEW_SHARE_NOTE", "PREVIEW_CONVERSION_PACKET"] as PlatformLeadArtifactType[]).map((type) => { const artifact = previewArtifactMap.get(type); return <article key={type} className="admin-card"><strong>{type.replaceAll("_", " ")}</strong><p className={artifact ? artifactTone(artifact.status) : "admin-chip admin-chip-warn"}>{artifact ? artifact.status.replaceAll("_", " ") : "Missing"}</p>{artifact ? <><p className="admin-muted">{artifact.title}</p><details><summary className="admin-muted">Review body</summary><p style={{ whiteSpace: "pre-wrap" }}>{artifact.body}</p></details></> : <p className="admin-muted">Generate starter packet or add this artifact manually.</p>}</article>; })}</div></section>
    ...
    <section className="admin-section admin-card"><h3>Preview Builds</h3><p className="admin-muted">Preview URLs are public-obscure, not confidential. Do not share with an owner until the packet, safety/rubric pass, and owner-share note are approved.</p><form action="/admin/platform-leads/previews" method="post" className="admin-form-stack"><input type="hidden" name="leadId" value={lead.id} /><input name="propertyName" defaultValue={lead.propertyName || lead.company || ""} required placeholder="Property name" /><input name="location" defaultValue={lead.propertyLocation || ""} required placeholder="Location" /><textarea name="sourceUrls" rows={3} defaultValue={lead.currentWebsite || ""} required placeholder="One source URL per line" /><input name="heroTitle" placeholder="Optional hero title" /><textarea name="positioning" rows={2} placeholder="Optional positioning line" /><button type="submit">Create Preview Build record</button></form>
      <ul className="admin-list">{lead.previewBuilds.map((preview) => { const gates = previewPacketReport(lead, preview); return <li key={preview.id}><strong>{preview.propertyName}</strong> <span className="admin-chip">{preview.status.replaceAll("_", " ")}</span><br/><Link href={`/p/${preview.slug}`}>/p/{preview.slug}</Link> <Link className="admin-muted" href={`/p/${preview.slug}?view=guest`}>guest view</Link><div className="admin-card" style={{ marginTop: 10 }}><strong>Packet gates</strong><p className={gates.readyBlockers.length ? "admin-chip admin-chip-warn" : "admin-chip"}>{gates.readyBlockers.length ? `Ready blocked: ${gates.readyBlockers.join("; ")}` : "Ready packet complete"}</p><p className={gates.shareBlockers.length ? "admin-chip admin-chip-warn" : "admin-chip"}>{gates.shareBlockers.length ? `Owner-share blocked: ${gates.shareBlockers.join("; ")}` : "Owner-share gate complete"}</p><p className={gates.promoteBlockers.length ? "admin-chip admin-chip-warn" : "admin-chip"}>{gates.promoteBlockers.length ? `Production promotion blocked: ${gates.promoteBlockers.join("; ")}` : "Promotion gate complete"}</p></div><form action="/admin/platform-leads/previews" method="post" className="admin-inline-form" style={{ marginTop: 12 }}><input type="hidden" name="action" value="generate-packet" /><input type="hidden" name="leadId" value={lead.id} /><input type="hidden" name="previewBuildId" value={preview.id} /><button type="submit" className="admin-button-secondary">Generate starter preview packet</button></form><details style={{ marginTop: 12 }}><summary className="admin-muted">Edit rendered sections / owner callouts</summary><form action="/admin/platform-leads/previews" method="post" className="admin-form-stack"><input type="hidden" name="action" value="content" /><input type="hidden" name="leadId" value={lead.id} /><input type="hidden" name="previewBuildId" value={preview.id} /><label className="admin-muted">Hero title<input name="heroTitle" defaultValue={preview.heroTitle || ""} /></label><label className="admin-muted">Positioning<textarea name="positioning" rows={2} defaultValue={preview.positioning || ""} /></label><label className="admin-muted">Sections JSON<textarea name="sectionsJson" rows={12} defaultValue={previewJson(preview.sections, DEFAULT_PREVIEW_SECTIONS)} /></label><label className="admin-muted">Owner callouts JSON<textarea name="ownerCalloutsJson" rows={7} defaultValue={previewJson(preview.ownerCallouts, DEFAULT_OWNER_CALLOUTS)} /></label><p className="admin-muted">Sections support kind, eyebrow, title/heading, body, items, imageUrl, and imageAlt. Keep owner/internal notes out of guest view by using owner callouts or Preview packet artifacts.</p><button type="submit">Save preview content</button></form><div className="admin-card" style={{ marginTop: 18 }}><h4>Manage existing sections</h4>{previewSections(preview.sections).length ? <ul className="admin-list">{previewSections(preview.sections).map((section, index) => <li key={index}><form action="/admin/platform-leads/previews" method="post" className="admin-form-stack"><input type="hidden" name="action" value="section" /><input type="hidden" name="leadId" value={lead.id} /><input type="hidden" name="previewBuildId" value={preview.id} /><input type="hidden" name="sectionIndex" value={index} /><label className="admin-muted">Kind<input name="sectionKind" defaultValue={section.kind || "custom"} /></label><label className="admin-muted">Eyebrow<input name="sectionEyebrow" defaultValue={section.eyebrow || ""} /></label><label className="admin-muted">Title<input name="sectionTitle" required defaultValue={section.title || section.heading || ""} /></label><label className="admin-muted">Body<textarea name="sectionBody" rows={3} defaultValue={section.body || ""} /></label><label className="admin-muted">Image URL<input name="sectionImageUrl" defaultValue={section.imageUrl || ""} /></label><label className="admin-muted">Image alt<input name="sectionImageAlt" defaultValue={section.imageAlt || ""} /></label><div className="admin-actions" style={{ flexWrap: "wrap" }}><button type="submit" name="sectionAction" value="update">Save section</button><button type="submit" name="sectionAction" value="up" className="admin-button-secondary">Move up</button><button type="submit" name="sectionAction" value="down" className="admin-button-secondary">Move down</button><button type="submit" name="sectionAction" value="delete" className="admin-button-secondary">Delete</button></div></form></li>)}</ul> : <p className="admin-muted">No rendered sections yet. Generate a starter packet or append one below.</p>}</div><form action="/admin/platform-leads/previews" method="post" className="admin-form-stack" style={{ marginTop: 18 }}><input type="hidden" name="action" value="add-section" /><input type="hidden" name="leadId" value={lead.id} /><input type="hidden" name="previewBuildId" value={preview.id} /><h4>Add one section without editing JSON</h4><label className="admin-muted">Kind<select name="sectionKind" defaultValue="custom"><option value="imageStory">imageStory</option><option value="signatureMoments">signatureMoments</option><option value="amenitiesConfidence">amenitiesConfidence</option><option value="locationGuide">locationGuide</option><option value="directBookingValue">directBookingValue</option><option value="missingInputs">missingInputs</option><option value="custom">custom</option></select></label><label className="admin-muted">Eyebrow<input name="sectionEyebrow" placeholder="Optional kicker" /></label><label className="admin-muted">Title<input name="sectionTitle" required placeholder="Section headline" /></label><label className="admin-muted">Body<textarea name="sectionBody" rows={4} placeholder="Source-backed section copy" /></label><label className="admin-muted">Image URL<input name="sectionImageUrl" placeholder="Optional permitted image URL" /></label><label className="admin-muted">Image alt<input name="sectionImageAlt" placeholder="Optional image alt text" /></label><button type="submit" className="admin-button-secondary">Append section</button></form></details><form action="/admin/platform-leads/previews" method="post" className="admin-inline-form"><input type="hidden" name="action" value="status" /><input type="hidden" name="leadId" value={lead.id} /><input type="hidden" name="previewBuildId" value={preview.id} /><select name="status" defaultValue={preview.status}>{PREVIEW_STATUSES.map((status) => <option key={status} value={status}>{status}</option>)}</select><button type="submit">Update</button></form></li>; })}</ul></section>
